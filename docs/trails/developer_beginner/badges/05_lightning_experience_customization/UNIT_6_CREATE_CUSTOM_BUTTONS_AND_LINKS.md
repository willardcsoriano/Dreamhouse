...
_Placement_ is where it splits:
...
- **`G3`, `C1` (`[CLI]` + `[GUI]`):** create and deploy the `WebLink` from the terminal first, _then_ switch to the browser to place it on the actions menu, then close out with retrieve → deploy.
